...
const TRANSITION_BOOM_SFX: AudioStream = preload(
	"res://assets/audio/sfx/ui/transition_full_black_boom.wav"
)
...
var pause_menu: ScrollerPauseMenu
var menu_audio: MenuAudioDirector
var screen_effects: ScreenEffects
var settings_pause_adapter: RuntimeTweakPauseAdapter = RuntimeTweakPauseAdapter.new()
var stage_registry: ScrollerStageRegistry = ScrollerStageRegistry.new()
...
func _ready() -> void:
	...
	var launcher: DebugTweakLauncher = DebugTweakLauncher.new()
	runtime_tweak_layer.add_child(launcher)
	launcher.pressed.connect(runtime_tweak_panel.toggle)
	menu_audio = MenuAudioDirector.new()
	add_child(menu_audio)
	pause_menu = ScrollerPauseMenu.new()
...
func _spawn_city_slice() -> void:
	var stage: ScrollerStageDefinition = stage_registry.at(active_stage_index)
	if stage == null or not stage.validation_errors().is_empty():
		_show_title()
		_show_content_error()
		return
	city_slice = stage.scene.instantiate() as CitySlice
	city_slice.stage_id = stage.stage_id
	city_slice.stage_district = stage.district
	city_slice.run_id = Crypto.new().generate_random_bytes(16).hex_encode()
	...
	city_slice.run_lifecycle.run_finished.connect(_on_stage_finished)
...
func open_gameplay_settings() -> bool:
	if city_slice == null or city_slice.game_over_active or title_transition_active:
		return false
	if city_slice.urban_siege == null or city_slice.urban_siege.pause_coordinator == null:
		return false
	if city_slice.urban_siege.pause_coordinator.is_paused() and not (pause_menu != null and pause_menu.lease.is_active()):
		return false
	_ensure_gameplay_settings()
	if gameplay_settings_screen.settings_open:
		return false
	if not settings_pause_adapter.acquire(city_slice, self, &"settings_dialog"):
		return false
	_gameplay_settings_pause_token = settings_pause_adapter.pause_token
	if pause_menu != null and pause_menu.lease.is_active():
		pause_menu.surface.hide()
...
func _release_gameplay_settings_pause() -> void:
	settings_pause_adapter.release()
	_gameplay_settings_pause_token = 0
	if is_instance_valid(pause_menu) and pause_menu.lease.is_active():
		pause_menu.surface.show()
...
func _show_title(restart_music: bool = false) -> void:
	...
	runtime_tweak_service.apply_audio_overrides()
...
func _validate_content() -> bool:
	startup_errors = stage_registry.validation_errors()
	startup_errors.append_array(CityDistrictCatalog.validation_errors())
	startup_errors.append_array(BossCampaignCatalog.validation_errors())
	startup_errors.append_array(EnemyArchetypeCatalog.validation_errors())
	startup_errors.append_array(DistrictMissionCatalog.validation_errors())
	startup_errors.append_array(DistrictPressureCatalog.validation_errors())
	startup_errors.append_array(DossierCatalog.validation_errors())
	startup_errors.append_array(WeaponShopCatalog.validation_errors())
	startup_errors.append_array(WeaponShopVisualCatalog.validation_errors())
	startup_errors.append_array(PlayerUpgradeAssembler.CATALOG.rebuild())
	return startup_errors.is_empty()


func _show_content_error() -> void:
	var dialog: AcceptDialog = AcceptDialog.new()
	dialog.title = L10n.t("boot.invalid_title")
	dialog.dialog_text = L10n.t("boot.invalid_body")
	dialog.ok_button_text = L10n.t("common.close")
	dialog.process_mode = Node.PROCESS_MODE_ALWAYS
	add_child(dialog)
	L10n.apply_locale_font(dialog)
	dialog.confirmed.connect(dialog.queue_free)
	dialog.canceled.connect(dialog.queue_free)
	dialog.popup_centered(Vector2i(480, 180))


func _on_stage_finished(completed: bool, _summary: RunSummarySnapshot) -> void:
	menu_audio.play_cue(&"victory" if completed else &"defeat")
	if completed and stage_registry.has_next(active_stage_index):
		call_deferred("_advance_stage")


func _advance_stage() -> void:
	if city_slice == null or not city_slice.game_over_active:
		return
...
func _close_transient_ui() -> void:
	if is_instance_valid(runtime_tweak_panel):
		runtime_tweak_panel.close()
	if is_instance_valid(gameplay_settings_screen) and gameplay_settings_screen.settings_open:
		gameplay_settings_screen.close_settings(false)
	if is_instance_valid(pause_menu):
		pause_menu.close()


func _recover_content_error() -> void:
