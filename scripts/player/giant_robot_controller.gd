...
var _keyboard_charge_held: bool = false
var _input_blocked_through_frame: int = -1
...
func _physics_process(delta: float) -> void:
	...
	if Engine.get_physics_frames() <= _input_blocked_through_frame:
		physics_step(0.0, delta)
		return
	var input_axis: float = Input.get_axis(&"move_left", &"move_right")
	if absf(virtual_move_axis) > absf(input_axis):
		input_axis = virtual_move_axis
	if Input.is_action_just_pressed(&"stomp"):
		_keyboard_charge_held = begin_attack_charge() > 0
	if Input.is_action_just_released(&"stomp") or (
		_keyboard_charge_held and not Input.is_action_pressed(&"stomp")
	):
		_keyboard_charge_held = false
...
func discard_modal_input() -> void:
	set_virtual_move_axis(0.0)
	_clear_move_tap()
	_keyboard_charge_held = false
	# action_release() does not erase a press edge from the same physics frame.
	# Resume motion immediately, but require fresh gameplay input after dismissal.
	_input_blocked_through_frame = Engine.get_physics_frames() + 1
