...
const GLASS_IMPACT_SFX: AudioStream = preload(
	"res://assets/audio/sfx/structural/glass_shatter.wav"
)
...
var stage_id: StringName = &"BUSINESS_ACT_1"
var stage_district: DistrictDefinition
var run_id: String = ""
var elapsed_run_seconds: float = 0.0
...
func _ready() -> void:
	...
	_build_hud()
	tutorial_run = gameplay_hud.first_run_tutorial.tutorial_active
	...
	if OS.is_debug_build() and _web_gameplay_smoke_requested():
...
func _process(delta: float) -> void:
	...
	if urban_siege != null and urban_siege.is_simulation_paused():
		return
	elapsed_run_seconds += maxf(delta, 0.0)
...
func _build_urban_siege() -> void:
	...
	urban_siege.setup(dependencies, stage_district if stage_district != null else CONTACT_DISTRICT)
