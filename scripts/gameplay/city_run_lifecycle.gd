...
func _boss_attempt_summary() -> RunSummarySnapshot:
	...
	var waves_cleared: int = clampi(city.encounter_director.phase_index + 1, 0, 1)
...
func _on_encounter_phase_changed(index: int, display_name: String) -> void:
	...
	city.gameplay_hud.set_siege_progress(index, 1, display_name, false)
...
func _on_district_completed() -> void:
	call_deferred("_finish_run", true)
...
func _on_continue_pressed() -> void:
	pass # Act 1 is the complete template campaign.
...
func _finish_run(completed: bool, ending_id: StringName = &"NONE") -> void:
	...
	var run_metrics: Dictionary = {
		"completed": completed, "run_id": city.run_id, "stage_id": city.stage_id,
		"duration_seconds": city.elapsed_run_seconds,
		"finished_unix_time": int(Time.get_unix_time_from_system()),
		"tutorial_run": city.tutorial_run, "debug_run": OS.is_debug_build(),
	}
	if not completed:
	...
	city.mobile_controls.set_controls_enabled(true)
	var waves_cleared: int = 1 if completed else clampi(
