...
const BUSINESS_PANORAMA: Texture2D = preload(
	"res://assets/city/parallax/districts/business_panorama.webp"
)
const FAR_TEXTURE: Texture2D = preload("res://assets/city/parallax/far_skyline.png")
const INFRA_TEXTURE: Texture2D = preload("res://assets/city/parallax/infrastructure.png")
const NEAR_TEXTURE: Texture2D = preload("res://assets/city/parallax/near_buildings.png")
...
var _foreground: BottomForeground


func _ready() -> void:
	_build_fixed_bands()
	_foreground = BottomForeground.new()
	_foreground.name = "BottomForeground"
...
func compensate_origin(offset: Vector2) -> void:
	_foreground.compensate_origin(offset)
	for child: Node in get_children():
		var band: Parallax2D = child as Parallax2D
		if band != null:
