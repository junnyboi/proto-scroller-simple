...
func _ready() -> void:
	...
	theme = RuntimeTweakTheme.compact_theme()
...
func open() -> bool:
	if not OS.is_debug_build() or visible or main == null or service == null or main.title_transition_active:
		return false
	var status: Dictionary = RuntimeTweakModalPolicy.entry_status(main.city_slice)
	if not bool(status.allowed):
		return false
	if not pause_adapter.acquire(main.city_slice, main):
		return false
	service.sync_audio_from_buses()
	visible = true
	refresh_locale()
	_refresh_rows()
	close_button.call_deferred(&"grab_focus")
	apply_responsive_layout(get_viewport_rect().size)
	call_deferred("apply_responsive_layout", get_viewport_rect().size)
	opened.emit()
	return true
...
func refresh_locale() -> void:
	...
	close_button.text = "X" if _portrait else L10n.t("common.close")
	close_button.accessibility_name = L10n.t("common.close")
...
func apply_responsive_layout(viewport_size: Vector2) -> void:
	...
	close_button.text = "X" if portrait else L10n.t("common.close")
	for row: TweakControlRow in rows:
		row.set_compact(portrait)
	_refresh_rows()
	frame.position = Vector2(margin, margin)
	frame.size = viewport_size - Vector2.ONE * margin * 2.0 - Vector2(0, 76)
...
func _on_row_value_requested(identifier: StringName, value: Variant) -> void:
	...
	if not bool(result.get("ok", false)):
		main.menu_audio.play_cue(&"invalid")
		save_status_label.text = L10n.t("tuning.validation.invalid_combination")
		save_status_label.tooltip_text = L10n.t("tuning.validation.invalid_combination")
...
func _on_persistence_state_changed(state: StringName, message: String) -> void:
	...
	if not message.is_empty():
		save_status_label.tooltip_text = L10n.t("tuning.save.not_saved")
