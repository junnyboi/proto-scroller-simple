...
const LANDSCAPE_ART: Texture2D = preload(
	"res://assets/ui/title_screen/command_deck_landscape.jpg"
)
const PORTRAIT_ART: Texture2D = preload(
	"res://assets/ui/title_screen/command_deck_portrait.jpg"
)
...
@onready var ui_mute_button: Button = %UiMuteButton
...
func _ready() -> void:
	...
	ui_mute_button.toggled.connect(_on_ui_mute_toggled)
	...
	ui_volume_slider.set_value_no_signal(float(volume_values[AudioVolumeSettings.Channel.UI]))
	...
	ui_mute_button.set_pressed_no_signal(
		AudioVolumeSettings.load_muted(AudioVolumeSettings.Channel.UI, audio_preference_path)
	)
...
func _update_audio_volume_values() -> void:
	...
	ui_volume_value.text = "%d%%" % int(round(ui_volume_slider.value))
...
func _apply_localized_text() -> void:
	...
	ui_volume_label.text = L10n.t("title.ui_volume")
...
func _apply_landscape_layout(viewport_size: Vector2) -> void:
	...
	_set_rect(
		briefing_toggle,
		Rect2(viewport_size.x - 430.0, viewport_size.y - (132.0 if OS.is_debug_build() else 72.0), 398.0, 58.0)
	)
...
func _apply_portrait_layout(viewport_size: Vector2) -> void:
	...
	_set_rect(
		briefing_toggle,
		Rect2(horizontal_center - 186.0, viewport_size.y - (150.0 if OS.is_debug_build() else 90.0), 372.0, 58.0)
	)
	_set_rect(
		briefing_tips_panel,
		Rect2(
...
func _on_ui_volume_changed(value: float) -> void:
	_save_volume(AudioVolumeSettings.Channel.UI, value)


func _on_ui_mute_toggled(muted: bool) -> void:
	_save_mute(AudioVolumeSettings.Channel.UI, muted)
