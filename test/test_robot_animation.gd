...
func test_modal_cancels_full_charge_without_releasing_a_strike() -> void:
	var city: CitySlice = await _spawn_city()
	var attacks: ContextualAttackController = city.contextual_attacks
	var presenter: RobotAnimationPresenter = (
...
func test_pause_freezes_then_resumes_dodge_and_player_locomotion() -> void:
	var city: CitySlice = await _spawn_city()
	var robot: GiantRobotController = city.robot
	var sprite: AnimatedSprite2D = _sprite(city)
	var presenter: RobotAnimationPresenter = (
	...
	assert_eq(robot.global_position.x, start_x)
	assert_true(city.urban_siege.pause_coordinator.release(pause_token))
	robot.physics_step(-1.0, 0.10)
...
func test_robot_mechanics_audio_is_qoa_fixed_and_frame_synchronized() -> void:
	var city: CitySlice = await _spawn_city()
	var robot: GiantRobotController = city.robot
	var sprite: AnimatedSprite2D = _sprite(city)
	var presenter: RobotAnimationPresenter = (
	...
	_assert_mono_cue(RobotAnimationPresenter.FOOTSTEP_SFX)
	_assert_mono_cue(RobotAnimationPresenter.SERVO_SFX)
	...
	_assert_mono_cue(RobotAnimationPresenter.DODGE_RECHARGED_SFX)
...
func test_dodge_recharge_sfx_plays_once_when_cooldown_completes() -> void:
	...
	assert_same(
		...
		load("res://assets/audio/sfx/robot/dodge_energy_recharged.wav")
	)
...
func _assert_mono_cue(stream: AudioStream) -> void:
